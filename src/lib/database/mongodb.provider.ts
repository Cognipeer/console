...
export class MongoDBProvider implements DatabaseProvider {
  ...
  async createGuardrail(
    ...
    const result = await db
      .collection(MongoDBProvider.guardrailsCollection)
      .insertOne(doc);
  ...
  async updateGuardrail(
    ...
    delete updateData._id;
    const result = await db
      .collection<IGuardrail>(MongoDBProvider.guardrailsCollection)
      .findOneAndUpdate(
        { _id: new ObjectId(id) },
        { $set: updateData },
        { returnDocument: 'after' },
  ...
  private static readonly guardrailEvalLogsCollection = 'guardrail_evaluation_logs';
  ...
  async listGuardrailEvaluationLogs(
    ...
    const docs = await db
      .collection(MongoDBProvider.guardrailEvalLogsCollection)
      .find(filter)
      .sort({ createdAt: -1 })
      .skip(options?.skip ?? 0)
      .limit(options?.limit ?? 50)
      .toArray();
    return docs as unknown as import('./provider.interface').IGuardrailEvaluationLog[];
  }
  ...
  async aggregateGuardrailEvaluations(
    ...
    const [totals] = await db.collection(MongoDBProvider.guardrailEvalLogsCollection).aggregate(pipeline).toArray();
    ...
    const [findingAgg] = await db.collection(MongoDBProvider.guardrailEvalLogsCollection).aggregate(findPipeline).toArray();
    ...
    const tsResults = await db.collection(MongoDBProvider.guardrailEvalLogsCollection).aggregate(tsPipeline).toArray();
