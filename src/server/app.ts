...
/**
 * Fastify's `trustProxy` accepts a boolean or an IP/CIDR list.
 ...
 * hardening this. Numeric hop counts fail closed because they cannot
 * authenticate the immediate peer. Non-numeric entries are IPs/CIDRs of the
 * actual trusted proxy/load balancer, and everything else falls back to the raw socket
 * address instead of a client-controlled header.
 */
export function resolveTrustProxyOption(trustedProxies: string[]): boolean | string[] {
