...
import { agentRunNotFoundErrorBody, serializeAgentRun } from '@/lib/services/agents/agentRunService';
import {
  ...
  app.get('/client/v1/agents/runs/:runId', withClientApiRequestContext(async (request, reply) => {
    try {
      ...
      return sendApiTokenError(reply, error)
        ?? reply.code(500).send({ error: { type: 'api_error', message: 'Internal error', code: 'internal_error' } });
    }
  }));
