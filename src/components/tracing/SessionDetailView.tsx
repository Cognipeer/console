...
export interface SessionDetailViewProps {
    sessionId: string;
    /**
     * Render inside another surface (the agent Sessions drawer) instead of as
     * a page: no page shell or back button, header collapses to one row.
     */
    embedded?: boolean;
    /** Extra header actions (e.g. Continue), placed before Docs / Refresh. */
    actions?: ReactNode;
}

/**
 * Agent Observability's session view — stat strip, event list and event
 * inspector for one traced run. Shared by the Tracing session page and the
 * agent Sessions drawer (one traced run per agent turn), so a turn reads the
 * same in both places.
 */
export default function SessionDetailView({ sessionId, embedded, actions }: SessionDetailViewProps) {
