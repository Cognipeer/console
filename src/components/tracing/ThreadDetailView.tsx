...
export interface ThreadDetailViewProps {
  threadId: string;
  /**
   * Render inside another surface (the agent Sessions drawer) instead of as a
   * page: no page shell / back button, and the header collapses to one row.
   */
  embedded?: boolean;
  /** Extra header actions next to the auto-refresh badge (e.g. Continue). */
  actions?: ReactNode;
  /** Shown in place of the "not found" error — an untraced session. */
  emptyFallback?: ReactNode;
}

/**
 * The Observability thread view: every traced turn of one conversation, the
 * selected turn's events and the event inspector. Shared by the Tracing
 * thread page and the agent Sessions drawer (agent turns are traced with
 * `threadId = conversationId`), so a session reads the same in both places.
 */
export default function ThreadDetailView({ threadId, embedded, actions, emptyFallback }: ThreadDetailViewProps) {
  ...
  const Shell = embedded ? EmbeddedShell : DetailShell;
  ...
  return (
    <Shell
      ...
      title={
        <>
          {/* Embedded, the host already titles the surface (the session name). */}
          {!embedded ? (
          <h1
              className="ds-h2"
...
/**
 * DetailShell's slots without the page around them — the drawer already has
 * its own frame, scroll and close button.
 */
    ...
    <Stack gap="md">
      <Group justify="space-between" align="flex-start" wrap="nowrap">
        <Stack gap={4} style={{ minWidth: 0 }}>
          <Group gap="xs" wrap="wrap">{title}</Group>
          {meta ? (
            <Group gap={6} wrap="wrap" style={{ fontSize: 12, color: 'var(--ds-text-muted)' }}>{meta}</Group>
          ) : null}
        </Stack>
        {actions ? <Group gap="xs" wrap="nowrap">{actions}</Group> : null}
      </Group>
      {children}
    </Stack>
  );
